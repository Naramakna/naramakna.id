...
async function pauseExpiredAds() {
  const lockKey = 'cron:pause-expired-ads';
  const lockValue = await RedisLock.acquire(lockKey, 180); // 3 min lock

  if (!lockValue) {
    console.log('⏭️  Another instance is already pausing ads, skipping...');
    process.exit(0);
  }
