...
export const AdminAds: React.FC = () => {
  ...
  const createAd = async () => {
    console.log('🚀 createAd called with formData:', formData);

    if (!formData.campaign_name) {
      setError('Campaign name is required');
      console.log('❌ Validation failed: campaign_name missing');
      return;
    }
    ...
    console.log('✅ Validation passed, creating ad...');

    try {
      setCreating(true);
      setError(null);

      // Get current user ID from localStorage or auth context (with error handling)
      let currentUserId = '1';
      try {
        const userStr = localStorage.getItem('user');
        if (userStr) {
          const user = JSON.parse(userStr);
          currentUserId = user?.ID?.toString() || '1';
        }
      } catch (e) {
        console.warn('⚠️ Could not parse user from localStorage, using default ID');
      }
