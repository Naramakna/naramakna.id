...
class MataElangController {
  ...
  static async getGallery(req, res) {
    try {
      ...
      // Get gallery details (fixed SQL - WHERE before LIMIT)
      const [galleries] = await connection.query(`
      ...
      // Get photos for this gallery (fixed SQL - WHERE before ORDER BY and LIMIT)
  ...
  static async getGalleryById(req, res) {
    try {
      ...
      const [photos] = await connection.query(`
        SELECT * FROM mata_elang_photos LIMIT 100
        WHERE gallery_id = ?
        ORDER BY sort_order ASC, id ASC
      `, [gallery.id]);
