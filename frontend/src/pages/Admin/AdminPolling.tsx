...
export const AdminPolling: React.FC = () => {
  ...
  const fetchPolls = async () => {
    try {
      const response = await fetch(buildApiUrl('polling/admin/all?limit=50'), {
        headers: {
      ...
      if (data.success) {
        ...
        const fallbackResponse = await fetch(buildApiUrl('polling/active?limit=50'), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const fetchTrendingPosts = async () => {
    try {
      const response = await fetch(buildApiUrl('polling/trending-candidates?limit=10'), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchArticles = async () => {
    try {
      const response = await fetch(buildApiUrl('content/posts-with-views?limit=50&status=publish'), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const closePoll = async (pollId: string) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`polling/${pollId}/close`), {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
